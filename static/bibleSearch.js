...
import {getBookIndex, getTotalChapters, getTotalLines, searchLine, getFullName} from "./bibleIndexes.js";
...
var validLine = function (mainLine, bookName, chapterIndex, line) {
    if (mainLine == -1) Searcher.onError("找不到書名", bookName);
    else if (mainLine == -2)
        Searcher.onError("章節不存在", bookName + " " + chapterIndex);
    else if (mainLine == -3)
        Searcher.onError(
        "句子不存在",
        bookName + " " + chapterIndex + ":" + line
        );
    else {
        return true;
    }
...
var findLines = function (startLine, endLine, bookName) {
    for (var i = startLine; i <= endLine; i++) {
        ...
        if (spaceIndex > 0) {
        Searcher.onTextLine(
            l.substring(0, spaceIndex),
            l.substring(1 + spaceIndex)
        );
        } else {
...
var find = function (
    ...
    chapterIndex2 =
        iChapterIndex2 === -1
        ? iChapterIndex1 === -1
            ? getTotalChapters(bookIndex)
            : iChapterIndex1
        : iChapterIndex2;
    line1 = iLine1 === -1 ? 1 : iLine1;
    line2 =
        iLine2 === -1
        ? getTotalLines(bookIndex, chapterIndex2)
        : iLine2;
    ...
    if (
        !validLine(startLine, bookName, chapterIndex1, line1) ||
        !validLine(endLine, bookName, chapterIndex2, line2)
    )
        return false; //validate lines
    //validations
    if (chapterIndex1 > chapterIndex2) {
        Searcher.onError(
        "章節順序錯誤",
        bookName + " " + chapterIndex1 + ": " + chapterIndex2
        );
        return false;
        ...
        Searcher.onError(
            "句子順序錯誤",
            bookName + " " + chapterIndex1 + ": " + line1 + " - " + line2
        );
        return false;
        }
    }
    ...
    if (chapterIndex1 === chapterIndex2) {
        ...
        else if (line1 != line2 && !(iLine1 === -1 && iLine2 === -1))
        title += ":" + line1 + "-" + line2;
    } else {
        if (iChapterIndex1 === -1 && iChapterIndex2 === -1);
        else if (iLine1 === -1 && iLine2 === -1)
        title += " " + chapterIndex1 + "-" + chapterIndex2;
        else
        title +=
            " " + chapterIndex1 + ":" + line1 + "-" + chapterIndex2 + ":" + line2;
    }
...
var indexSearch = function (query) {
    ...
    var re =/\s*•?\s*([0-9]+)\s*(?:[:：]\s*([0-9]+)(?:\s*[-－]\s*([0-9]+))?((?:\s*[,，]\s*)*))?/g;
    var m;
